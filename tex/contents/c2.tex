...
\item
\verb+\mybold{}+ --- \mybold{another type of bold}. (The \verb+\mybold+ command is defined in the \verb+tex/config/after-begin.tex+ file.)
\end{itemize}
...
A table footnote is used in \Cref{tab:table-with-footnote}.

\input{tex/contents/tab/table-with-footnote}
...
\Cref{tab:a-long-table} presents a long table using the \verb+longtable+ package. This table can span multiple pages. The \verb+\afterpage+ commands forces the table to start at the top of a page.

\afterpage{\input{tex/contents/tab/a-long-table}}
